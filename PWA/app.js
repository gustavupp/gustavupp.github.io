...
function addTodoItem(e) {
  ...
  if (INPUT_VALUE && !editMode) {
    ...
    NEW_INDIVIDUAL_ITEM.innerHTML = `<div class="title">${INPUT_VALUE}</div>
...
function displayCurrentList() {
  ...
  localStorageList = localStorageList.forEach(function (item) {
    ...
    //create child elements and append to NEW_INDIVIDUAL ITEM
    NEW_INDIVIDUAL_ITEM.innerHTML = `<div class="title">${item.value}</div>
        <div class="btn-container">
            <button type="button" class="edit-btn icon">
                <i class="fas fa-edit"></i>
            </button>
...
function removeFromLocalStorage(ID) {
  ...
  localStorageList = localStorageList.filter(function (item) {
    //modifies the original array excluding the matching item.id
    if (item.id !== ID) {
      return item
    }
  })
...
function getLocalStorage() {
  ...
  return localStorage.getItem('localStorageList')
    ? JSON.parse(localStorage.getItem('localStorageList'))
    : []
}
